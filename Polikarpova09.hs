...
-- Задача 2 ----------------------------------------- 
brs:: Parser ()
brs = many braces >> return ()
    where braces = choice [between (char q) (char w) brs | (q,w) <- [('(',')'),('[',']'),('{','}')]]

balance :: String -> String
balance str = case parse (brs >> eof) "" (filter (/=' ') str) of
                       Left  _ -> "No balanced"
                       Right _ -> "Is balanced"
...
relOp :: Parser Bop  
relOp = try (oper ">=" Ge ) <|> try (oper ">"  Gt ) <|> try (oper "<=" Le ) <|> try (oper "<"  Lt ) <|> try (oper "==" Eql)
